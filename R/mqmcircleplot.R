...
mqmplot.circle <- function(cross, result, highlight=0, spacing=25, interactstrength=2,legend=FALSE, verbose=FALSE, transparency=FALSE){
  ...
  if(!legend){
    ...
    if(any(class(result)=="scanone") || highlight > 0){
      ...
      legend("bottomright",c("Lod 3","Lod 6","Lod 9","Lod 12"),col=gray(0.5-(0.4*(c(3,6,9,12)/min(12,max(result[,3],na.rm=T))))),pch=19,lwd=0,pt.cex=c(1,2,3,4))
...
drawcirculargenome <- function(result,lodmarkers=FALSE,spacing=50){
  ...
  for(x in 1:nrow(result)){
    ...
    if(lodmarkers){
      ...
      val <- (0.4*(result[x,3]/max(result[,3],na.rm=T)))
      if(is.na(val)){
        color <- gray(0)
      }else{
        color <- gray(0.5-val)
      }
      points(locationtocircle(result,result[x,1],result[x,2],spacing=spacing),pch=20,col=color,cex=size)
    }else{
