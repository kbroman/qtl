...
fitplot.scanonevar <- function (cross, name.of.marker.to.plot, varscan) {
	...
	if (units(varscan) == 'lods') {
		chr.peaks <- dplyr::filter(varscan, chr == name.of.chr.with.marker) %>%
	...
	if (mles$chrtype == 'A') {
		...
		aa.mean <- mles$mean.baseline - mles$mean.add.effect
		ab.mean <- mles$mean.baseline
		bb.mean <- mles$mean.baseline + mles$mean.add.effect

		aa.sd <- mles$var.baseline - mles$var.add.effect
		ab.sd <- mles$var.baseline
		bb.sd <- mles$var.baseline + mles$var.add.effect

		if (attr(varscan, 'dom')) {
			ab.mean <- ab.mean + mles$mean.dom.effect
			ab.sd <- ab.sd + mles$var.dom.effect
		}
	}
	...
	if (mles$chrtype == 'X') {
		...
		ab.mean <- with(mles, mean.baseline + 0.5*mean.add.effect)
		bb.mean <- with(mles, mean.baseline + mean.add.effect)

		aa.sd <- mles$var.baseline
		ab.sd <- with(mles, var.baseline + 0.5*var.add.effect)
		bb.sd <- with(mles, var.baseline + var.add.effect)
	}
