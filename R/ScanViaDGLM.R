...
#### loop through markers (including pseudomarkers) ####
  for (marker.idx in 1:length(marker.names)) {
    ...
    if (ncol(marker.genoprobs) == 3) {
      mapping.df$mean.QTL.add <- mapping.df$var.QTL.add <- AdditiveCoefficientFrom3Genoprobs(marker.genoprobs)[perm]
      mapping.df$mean.QTL.dom <- mapping.df$var.QTL.dom <- DominantCoefficientFrom3Genoprobs(marker.genoprobs)[perm]
    }
    if (ncol(marker.genoprobs) == 2) {
      mapping.df$mean.QTL.add <- mapping.df$var.QTL.add <- AdditiveCoefficientFrom2Genoprobs(marker.genoprobs)[perm]
