...
scanonevar.varperm <- function(cross, pheno.col=1, mean_covar = NULL, var_covar = NULL,
															 dom = FALSE, maxit = 25 , tol=1e-6, quiet=TRUE, chrs,
															 job.num = 1, num.var.perms = 5)
{
	...
	all.results <- vector(mode = 'list', length = num.var.perms)

	for (perm.num in 1:num.var.perms) {
...
# 				}
			...
			w <- names(map)
			o <- grep("^loc-*[0-9]+",w)
			perm.idx <- (job.num - 1)*num.var.perms + perm.num
