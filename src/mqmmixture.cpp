...
//		Rprintf("INFO: ML\n");
  ...
  if (fitQTL=='n') {
    ...
    for (j=0; j<Nloci; j++) {
      ...
      if ((position[j]==MLEFT)||(position[j]==MMIDDLE)) {
        for (i=0; i<Naug; i++) {
          calc_i = prob(loci, r, i, j, loci[j+1][i], CF2, 0, 0);
          Ploci[i]*= calc_i;
        }
      }
    }
      ...
      if ((position[j]==MLEFT)||(position[j]==MMIDDLE)) {
        if ((cofactor[j]<=MH)&&(cofactor[j+1]<=MH))
          ...
          for (i=0; i<Naug; i++) { // QTL==MAA What is the prob of finding an MAA at J=1
            calc_i = prob(loci, r, i, j, MAA, crosstype, 0, 0);
            Ploci[i]*= calc_i;
            // QTL==MH
            calc_i = prob(loci, r, i, j, MH, crosstype, 0, 0);
            Ploci[i+Naug]*= calc_i;
            // QTL==MBB
            calc_i = prob(loci, r, i, j, MBB, crosstype, 0, 0);
            Ploci[i+2*Naug]*= calc_i;
          }
        else // locus j == QTL
          for (i=0; i<Naug; i++) { // QTL==MAA
            calc_i = prob(loci, r, i, j+1, MAA, crosstype, -1, 0);
            Ploci[i]*= calc_i;
            // QTL==MH
            calc_i = prob(loci, r, i, j+1, MH, crosstype, -1, 0);
            Ploci[i+Naug]*= calc_i;
            // QTL==MBB
            calc_i = prob(loci, r, i, j+1, MBB, crosstype, -1, 0);
            Ploci[i+2*Naug]*= calc_i;
          }
      }
    }
