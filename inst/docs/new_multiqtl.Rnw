...
Karl W Broman, 16 January 2008

\bigskip

R/qtl version 1.08 includes a number of new functions to simplify the
exploration of multiple-QTL models.  

The ``Brief tour of R/qtl'' document contains a brief description of
these functions (see Example 5 in that document); here, we provide a more thorough
explanation. 
...
\code{keeplodprofile=TRUE}, one may use another new function,
...
Finally, we added several functions for manipulating a QTL object
...
imputations; more is always better, but it slows the work down
considerably.  
<<simgeno,eval=FALSE>>=
hyper <- sim.geno(hyper, step=2, n.draws=128, err=0.001)
@ 
<<runsimgeno,echo=FALSE>>=
...
positions); \code{fitqtl} requires a column of phenotypes, rather than
a cross object, and so we use \code{pull.pheno} to pull out the data
for the first phenotype, blood pressure.  Note that we use
...
out.fq <- fitqtl(phe, qtl=qtl, formula=y~Q1+Q2+Q3*Q4)
summary(out.fq)
@ 
...
drop-one analysis.
<<fitqtl2>>=
out.fq2 <- fitqtl(phe, qtl=rqtl, formula=y~Q1+Q2+Q3*Q4, dropone=FALSE)
summary(out.fq2)
@ 

The LOD score comparing the full model to the null model has increased
by \Sexpr{round(out.fq2[[1]][1,4] - out.fq[[1]][1,4],1)}, from
\Sexpr{round(out.fq[[1]][1,4],1)} to \Sexpr{round(out.fq2[[1]][1,4],1)}.  

If \code{refineqtl} is run with the argument
\code{keeplodprofile=TRUE}, one saves the LOD traces at that last
iteration, which can then be plotted with \code{plotLodProfile}, as
follows. See Fig.~\ref{fig:lodprofile}.
<<lodprofile,eval=FALSE>>=
rqtl <- refineqtl(hyper, qtl=rqtl, formula=y~Q1+Q2+Q3*Q4, verbose=FALSE, 
                  keeplodprofile=TRUE)
plotLodProfile(rqtl)
@ 
...
The syntax of the function is similar to that of \code{fitqtl}, taking
a phenotype, QTL object, and model formula.  The output is a table of
results similar to that provided by the drop-one analysis of
\code{fitqtl}. 
<<addint>>=
addint(phe, qtl=rqtl, formula=y~Q1+Q2+Q3*Q4)
@ 
...
additive.  The syntax of \code{addqtl} is similar to that of
\code{refineqtl}: it requires the cross object, \code{hyper}, as well
as a QTL object and a formula.
<<addqtl,eval=FALSE>>=
out.aq <- addqtl(hyper, qtl=rqtl, formula=y~Q1+Q2+Q3*Q4)
@ 
<<runnaddqtl,echo=FALSE>>=
...
is 1.64, indicating relatively weak evidence for a second QTL on chr 1.
...
\code{lod.2v1a} is \Sexpr{round(summary(out.ap2)[3,7],2)}, indicating little
...
<<addtoqtl>>=
rqtl <- addtoqtl(hyper, rqtl, 1, 73.3)
rqtl
@ 
...
position (say to 43.3 cM rather than 67.8 cM), we may use
\code{replaceqtl}, as follows.
<<replaceqtl>>=
rqtl <- replaceqtl(hyper, rqtl, 1, 1, 43.3)
rqtl
@ 

If we wish to reorder the QTL (e.g., according to their map positions), 
we may use \code{reorderqtl}, as follows. 
<<reorderqtl>>=
rqtl <- reorderqtl(rqtl, c(1,5,2:4))
rqtl
@ 
