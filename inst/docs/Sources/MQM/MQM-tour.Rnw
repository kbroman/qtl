...
The (rather famous) mouse hypertension dataset\cite{sugiyama}, distributed with
...
set to 1.0 (omitting to set it would issue a warning and it would be automatically set to 1.0).
As an alternative we could also have used the fill.geno function from R/qtl.
