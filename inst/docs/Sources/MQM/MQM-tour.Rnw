...
creates a list consisting of multiple cofactors that can be used for backward elimination. Iteratively 
the MQM algorithm analyses all the markers and drops the least informative cofactor from the model. This step is repeated
until a limited number of significant cofactors remains.
