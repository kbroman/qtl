...
listeriaaug <- mqmaugment(listeria, minprob=1.0)
result_no_cofactors <- mqmscan(hyperaug)
result_listeria_no_cofactors <- mqmscan(listeriaaug)
@
...
listeria <- calc.genoprob(listeria, step=5)
result_scanone <- scanone(hyper)
result_listeria_scanone <- scanone(listeria)
@
...
\end{figure}
\begin{figure}
<<NoCofsListeria,eval=FALSE>>=
plot(result_listeria_no_cofactors, result_listeria_scanone, col=c("black", "green"), lty=1:2)
@ 
\caption{Listeria mqm versus scanone}
\end{figure}
