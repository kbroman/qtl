...
<<augmentMinProb>>=
maug10minprob <- mqmaugment(msim10,minprob=0.001,verbose=T)
maug10minprobImpute <- mqmaugment(msim10,minprob=0.001,unaugmentable="impute",verbose=T)
# nind(maug)
nind(maug10minprob)
...
<<augment5>>=
result5perc <- mqmscan(maug5)
result10perc <- mqmscan(maug10)
result80perc <- mqmscan(maug80)

resultsone5 <- scanone(msim5)
resultsone10 <- scanone(msim10)
resultsone80 <- scanone(msim80)
@

<<augment6,eval=FALSE,echo=FALSE>>=
op <- par(mfrow=c(2,2))
plot(result5perc,result10perc,result80perc,col=c("green","blue","black"),main="MQM missing data")
legend("topleft",c("MQM 5%","MQM 10%","MQM 80%"),col=c("green","blue","black"),lwd=1)
plot(resultsone5,result5perc,main="5% missing", col=c("red", "green"))
legend("topleft",c("scanone","MQM"),col=c("red","green"),lwd=1)
plot(resultsone10,result10perc,main="10% missing", col=c("red", "blue"))
legend("topleft",c("scanone","MQM"),col=c("red","blue"),lwd=1)
plot(resultsone80,result80perc,main="80% missing", col=c("red", "black"))
...
result_maug_no_cofactors <- mqmscan(maug)
@

Scan for \qtl\ with \code{mqmscan}.
We compare \code{mqmscan} with \code{scanone}, for which
one first calculates conditional QTL genotype probabilities via
\code{calc.genoprob} (see section \ref{augmentation}).
...
plot(result_maug_no_cofactors, result_multi_scanone, col=c("black", "green"), lty=1:2)
@ 
...
<<>>=
just_markers <- mqmextractmarkers(result_maug_no_cofactors)
@ 
...
nearest to the peak on chromosome 5 and perform a new \code{mqmscan} (see
figures \ref{Cofactor4} and \ref{Cofactor4b}).

\intro{Add marker GH.117C (chromosome 5, at 35 \cM) as a cofactor}

<<>>=
max(result_maug_no_cofactors)
find.marker(maug,chr=5,pos=35)
multitoset <- find.markerindex(maug,"GH.117C")
multicofactorlist <- mqmsetcofactors(maug,cofactors=multitoset)
result_multi_1 <- mqmscan(maug, multicofactorlist)
@
...
<<Cofactor4multi>>
@
\caption{\Atintro. mqmscan after a cofactor is added at the top scoring marker of chromosome 5. During the analysis it is kept in the
...
chromosome 5. This can be shown by plotting these new
...
multicofactorlist <- mqmsetcofactors(maug,cofactors=multitoset)
result_multi_2 <- mqmscan(maug, multicofactorlist)
@
% -----------
...
plot(result_maug_no_cofactors, result_multi_1, result_multi_2,
     col=c("black","red","blue"), lty=1:3)
@ 
...
<<eval=FALSE>>=
multicofactorlist <- mqmautocofactors(maug,50)
autoresultmulti <- mqmscan(maug, multicofactorlist)
multicofactorlist <- mqmsetcofactors(maug,5)
backwardresultmulti <- mqmscan(maug, multicofactorlist)
@
...
<<FigLowAlpha,eval=FALSE>>=
multiresult <- mqmscan(maug, multicofactorlist, cofactor.significance=0.002)
par(mfrow = c(2,1))
...
<<>>=
require(snow)
results <- mqmpermutation(maug,scanfunction=mqmscan,cofactors=multicofactorlist,n.cluster=2,n.perm=25,batchsize=25)
@
\begin{figure}
<<fig=TRUE>>=
