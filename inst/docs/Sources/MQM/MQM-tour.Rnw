...
changes (see Figure~\ref{AtNoCofs}). \textbf{[\FIXME this figure was
  commented out.]}
...
default values. Figure~\ref{AtNoCofs} \textbf{[\FIXME this figure was
  commented out.]} displayed a large QTL peak on chromosome 5
...
content per marker (red), the mapped \qtl\ (black) and the \qtl\ information score (\code{info} blue) which is calculated by multiplying marker information content with \qtl\ LOD score.
