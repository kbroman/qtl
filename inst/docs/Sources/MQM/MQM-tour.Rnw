...
remove the original genotype/individual from the analysis.  Note that you can
opt to use \code{fill.geno}'s imputation method, instead of augmentation, when
too many individuals are dropped because of missing data.
