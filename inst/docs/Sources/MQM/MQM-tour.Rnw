...
low it is possible that an individual cannot be augmented enough, is removed
from the dataset (to prevent memory overflow), and genotype information is lost.
