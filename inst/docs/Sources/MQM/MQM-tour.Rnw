...
\section{Unsupervised backward elimination\label{backelim}}
...
<<eval=FALSE>>=
autocofactorlist <- mqmautocofactors(maug,50)
mqm_auto <- mqmscan(maug, autocofactorlist)
...
par(mfrow = c(2,1))
plotcofactors(maug,autocofactorlist)
plotcofactors(maug,multicofactorlist)
@ 
...
Automatic marker selection (bottom). We see the difference in placing the initial 
cofactors between the two methods (top: mqmautocofactors, bottom: mqmsetcofactors) \label{ManualAutoStart}}
\end{figure}
...
place a cofactor at chromosome 1 (see Figure \ref{ManualAutoStart}), while this one is missed by mqmsetcofactors
