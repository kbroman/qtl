...
by selecting cofactors automatically. When the sample size allows, it is normal
procedure to set a cofactor at every marker. However current day experiments
handle less individuals than genetic locations (markers),therefor only a subset
of locations can be examined, ( because of lack of sample size (we can set as
many cofactors as we have individuals-12)).  The functions:
"\code{mqmsetcofactor}" and "\code{mqmautocofactors}" both create lists
consisting of multiple cofactors that can be used for backward elimination.
\code{mqmautocofactor} is prefered when dealing with low samplesizes, because
it takes the underlying marker density into account. See
...
\code{mqmautocofactor} to set 50 cofactors.  After cofactor selection
iteratively the MQM algorithm analyses all the markers and drops the least
informative cofactor from the model. This step is repeated until a limited
number of significant cofactors remains.  From Figure~\ref{ManualAuto} we see
that when taking marker density into account we get an extra marker that seems
significant when used as cofactor, compared to the naive approach where we miss
this marker because we set cofactors every 5th marker.

After backward elimination \mqm\ scans each chromosome using the model that
includes the retained cofactors. Here we set 50 cofactors using the automatic
cofactor routine, and we use the naive implementation of setcofactors to set a
cofactor at every fifth marker using \code{mqmsetcofactors}, and assess which
chromosomes may be implicated in the various traits in our multtrait
metabolomics dataset (results in Figures~\ref{Backward1} and \ref{Backward2}):
...
Automatic marker selection (bottom). We see the difference in placing the
initial cofactors between the two methods (top: mqmautocofactors, bottom:
mqmsetcofactors) \label{ManualAutoStart}}
\end{figure}
