...
the various traits in our multtrait metabolomics dataset pressure (results in Figures~\ref{Backward1} and \ref{Backward2}):
...
<<ManualAuto>>
@
\caption{\Atintro after introducing cofactors at every fifth (left) and Automatic marker selection (Right) AFTER backward elimination\label{ManualAuto}}
\end{figure}
