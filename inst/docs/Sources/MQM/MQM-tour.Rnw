...
\newcommand{\Atintro}{\At\ RIL mQTL dataset (multitrait) with 24 metabolites as phenotypes\cite{Keurentjes:2006}}
...
drops an individual if there are too many possible genotypes. You can opt to
use \code{fill.geno}'s imputation method, instead, when too many individuals
are omitted, because of missing data. A future version of \code{mqmaugment} may
take an automatic approach, using imputation with those individuals that cannot be reasonably augmented.
...
(a future release of \mqm\ may use imputation method for dropped individuals). Increasing minprob 
(towards a value of 1.0) can keep these individuals inside the dataset too, a good rule of thumb may be to 
set \code{minprob} to the percentage of data missing. A value of \code{minprob=1.0} 
prevents the dropping of individuals, making augmentation behave similar to \code{fill.geno}'s 
imputation method, though with different selected genotypes}.  
Use \code{verbose=TRUE} to get more feedback on the augmentation
routine and to check how many
individuals are dropped. First we try augmentation with \code{minprob=1.0} (Figure~\ref{augment1}):
...
scored (with errors at) 117 markers\cite{Koornneef:1998}. The experiment
...
After augmentation check how many individuals still remain in the dataset.
Normally, choose a value of \code{minprob} so no individuals are dropped.  Next
...
traits from a RIL population of \At\cite{Keurentjes:2006} (see also section
...
<<Backward1multi>>
@
\caption{backward on multi after introducing cofactors at every fifth marker and backward elimination\label{Backward1}}
\end{figure}
...
<<Backward2multi>>
@
\caption{backward multi versus scanone \label{Backward2}}
\end{figure}
...
<<FigLowAlpha>>
@
\caption{Multi lower P (no change in model) \label{FigLowAlpha}}
\end{figure}
...
distribution. For example, with a dataset of 100 Arabidopsis, when comparing two
distinct phenotypes at a marker location, we have most power when both groups
are equally divided into 50 Arabidopsis, and virtually no power when we have one
mouse versus a group of 99 Arabidopsis. We can multiply the estimated \qtl\ effect by
