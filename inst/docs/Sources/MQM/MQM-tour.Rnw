...
set \code{minprob} to the percentage of data missing. Furthermore augmentation cannot handle selective genotyped 
datasets, these will always be handled with minprob=1.0 (a warning will be issued). A value of \code{minprob=1.0} 
...
The hypertension dataset\cite{sugiyama} (hyper), distributed with
R/qtl, which is a backcrossed (BC) offspring of mice with two listed traits:
blood pressure (bp) and sex. Here we analyse blood pressure in the hyper dataset using both
\code{scanone}
single marker \qtl\ analysis and \code{mqmscan} multiple \QTL\ model mapping.
First \emph{complete} missing data using the \code{mqmaugment} function (see also section \ref{augmentation}). 
Because the hyper dataset has been selectively genotyped the \code{minprob} parameter of the augmentation is 
set to 1.0 (omitting it here would issue a warning and automatically set to 1.0).

% \texttt{qtl.mqm} help page into a \LaTeX{} document

\intro{Use \code{scanone} and \code{mqmscan} after filling missing data with \code{mqmaugment} minprob=1.0}
<<>>=
data(hyper)
data(multitrait)
...
\code{calc.genoprob} (see also section \ref{augmentation}).
...
Figure \ref{NoCofs} shows that, with default parameters, the results from 
