...
plot(mqm_co0, m_one, col=c("green", "black"), lty=1:2)
@ 
...
plot(mqm_co1, m_one, col=c("green","black"), lty=2:1)
@
...
plot(mqm_co2, mqm_co1, col=c("blue","green"), lty=1:2)
@ 
...
chromosome. (\code{mqmscan} 2 cofactor (blue) differs from \code{mqmscan} 1 cofactors (green, dashed))
...
In our example we have already have a small model, so we don't really expect to
lose the two QTL on chromosome 4 and chromosome 5. And, indeed, when decreasing
