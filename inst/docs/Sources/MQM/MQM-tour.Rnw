...
(single-\qtl\ analysis) and \code{mqmscan} (multiple \QTL\ model mapping).
...
<<>>=
data(multitrait)
maug_1 <- mqmaugment(multitrait,minprob=1.0)
mqm_1  <- mqmscan(maug_1)
@
...
plot(m_one, mqm_1, col=c("black", "green"), lty=1:2)
legend("topleft",c("scanone","MQM"),col=c("black","green"),lwd=1)
@ 
