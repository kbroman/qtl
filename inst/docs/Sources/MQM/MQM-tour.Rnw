...
than other many methods.  R/qtl's \mqm\ is faster than other implementations and 
...
for every marker for every 
individual determined), however in the real world datasets are often incomplete. That is, genotype
