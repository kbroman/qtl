...
We begin by removing data (5%, 10% and 80%) from the genotypes
<<augment3>>=
data(multitrait)
...
want to include more unlikely genotypes. This leads to the situation 
...
\code{fitqtl} routines from R/qtl.
\code{mqmgetmodel} can only be used after backward elimination 
as it requires a list of cofactors. The resulting model can also
 be used to obtain the location and name of the significant cofactors.
