...
First we explore without augmenting genotypes (as described in section
\ref{augmentation}), using \code{minprob=1.0}.
...
<<Cofactor4multi>>
@
\caption{\Atintro. A cofactor is added and kept in the
...
<<Cofactor4bMULTI>>
@
\caption{\Atintro after introducing a cofactor on chromosome 5 (GH.117C) we see a light increase in LOD scores. \label{Cofactor4b}}
\end{figure}
...
@
\caption{\Atintro Comparison between scanning using 0,1 and 2 cofactor(s)}
\end{figure}
% ---------------------
