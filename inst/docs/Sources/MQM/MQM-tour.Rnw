...
Most real world datasets are incomplete. That is, genotype
% or phenotype -- removed as augmentation does not handle missing
% phenotypes
...
When encountering a missing marker (possible genotypes {\bf A} and {\bf B} in a
RIL or BC) all possible genotypes at the missing location are created.  Thus at
...
augmentation round as the maxumimum of augmented individuals (the parameter \code{maxaugind}) is quickly reached.
...
data, like in the case of selective genotyped datasets (for example the Mouse hyper
...
and is moved to the second round of augmentation (see description above).
...
Then some individuals are moved to the second round then missing data is either filled using the most likely marker 
or using imputation. We can also choose to drop the individuals entirely from the rest of the analysis, this is however 
not advisable because usable information is lost.

<<augmentMinProb>>=
multiaug10minprob <- mqmaugment(multimissing10,minprob=0.001,verbose=T)
multiaug10minprobImpute <- mqmaugment(multimissing10,minprob=0.001,unaugmentable="impute",verbose=T)
nind(multiaug)
nind(multiaug10minprob)
nind(multiaug10minprobImpute)
@
...
op <- par(mfrow=c(2,2))
plot(result5perc,result10perc,result80perc,main="MQM missing data")
legend("topleft",c("MQM 5%","MQM 10%","MQM 80%"),col=c("black","green","red"),lwd=1)
plot(resultsone5,result5perc,main="5% missing", col=c("blue", "black"))
legend("topleft",c("scanone","MQM"),col=c("blue","red"),lwd=1)
plot(resultsone10,result10perc,main="10% missing", col=c("blue", "green"))
legend("topleft",c("scanone","MQM"),col=c("blue","red"),lwd=1)
...
Scan for \qtl\ with \code{mqmscan} (using default parameters).
...
By default \MQM\ introduces pseudo markers at fixed intervals, unlike
...
at 35 \cM. So account for that by setting a cofactor at the marker
...
\intro{Automatic cofactor selection followed by backward elimination}
Example on how-to set cofactors using \code{mqmautocofactors} and \code{mqmsetcofactors}:
<<eval=FALSE>>=
...
<<Backward1multi>>
@
\caption{backward elimination on \At after introducing cofactors at every fifth marker and backward elimination\label{Backward1}}
\end{figure}
...
the significance level also has the advantage that we can be more sure of 
the \qtl we report. In our example we already found a small model so we don't 
expect to loose the two QTLs on chromosome 4 and chromosome 5. And indeed when deceasing 
the \code{cofactor.significance} no additional cofactors are dropped from the model (See Figure~\ref{lowalpha})
