...
\begin{itemize}
\item Load an experiment in R using R/qtl data types
...
Most real world datasets are incomplete. That is, genotype or phenotype information is
...
(estimated) recombination rate, a probability is attached to all possible genotypes.  With \mqm\ all
possible genotypes with a probability above minprob are considered.  When encountering missing marker 
in a RIL (possible genotypes A and B) all possible genotypes at the missing location are created.
Thus at the missing location two `individuals' are created in the augmentation step, one with
genotype A, and one with genotype B (given that both have a probability of occuring more than minprob).
And the probability is attached to both genotypes. The combined probabilities of all missing marker 
locations combined tells us whether a genotype is likely, or unlikely. So if a RIL is not missing 
any markers it is kept in the set as a single individual with P=1.0, if there is one 
missing marker and genotype A is as likely as B then two individuals are created each with P=0.5.
...
single (and probably fictional individual) for further analysis. 

The downside of augmentation is that the addition of many possible genotypes can
exceed available computer memory.  Currently, augmentation
moves an individual to a second augmentation round. In the second augmentation round 
the user can specify what needs to be done with these genotypes: Fill in the most likely, 
multiple imputation or removal from the analysis. This will only happen to individuals with 
a lot of missing data (leading to too many possible genotypes). You can opt to
use \code{fill.geno}'s imputation method, also.
...
low may result in moving a lot of individuals to the second augmentation round as the parameter \code{maxaugind} is quickly
reached. Increasing minprob (towards a value of 1.0) can keep individuals with more missing data inside the first 
augmentation round, a good rule of thumb may be to set \code{minprob} to the percentage of data missing. A value of \code{minprob=1.0} 
makes augmentation behave similar to \code{fill.geno}'s imputation method, though with slightly different genotypes.  
Use \code{verbose=TRUE} to get more feedback on the augmentation routine and to check how many
individuals are moved to the imputation / mostLikely / removal. First we use the augmentation with \code{minprob=1.0}, 
so basically doing an fill.geno (Figure~\ref{augment1}):
...
(\code{mqmaugment} with \code{minprob=1.0} means only a `most likely' individual is
used and
no real expansion of the dataset takes place, similar to \code{fill.geno}).\label{augment1}}
\end{figure}

With a lower \code{minprob}, more {\bf unlikely} genotypes are kept, and the
...
\label{multitrait}
A real dataset (\code{multitrait}), which contains 24 metabolite
traits from a RIL population of \At.  The \code{multitrait} dataset comes with
R/qtl (\code{data(multitrait)}) and is part of the \At\ RIL selfing experiment
...
increase the \code{minprob} parameter.  When the \code{minprob} parameter is set too
low it is possible that an individual cannot be augmented, this individuals is the 
moved to the second round for augmentation and is augmented using (based on the user settings): imputation / mostLikely / removalremoved
from the dataset (to prevent memory overflow), So no genotype information is lost.
...
scan for QTLs inside the crossobjects with \code{mqmscan} and the single QTL
...
plot(resultsone5,result5perc,main="5% Missing", col=c("blue", "red"))
legend("topleft",c("scanone","mqm"),col=c("blue","red"),lwd=1)
plot(resultsone10,result10perc,main="10% Missing", col=c("blue", "red"))
legend("topleft",c("scanone","mqm"),col=c("blue","red"),lwd=1)
plot(resultsone80,result80perc,main="80% Missing", col=c("blue", "red"))
legend("topleft",c("scanone","mqm"),col=c("blue","red"),lwd=1)
@ 
...
<<augment6>>
@
\caption{\Atintro. Effect of missing data on \code{mqmscan} and \code{scanone} methods
\label{augment6}}
\end{figure}
\clearpage
...
Here we analyse the \code{multitrait} dataset using both \code{scanone} single
marker \qtl\ analysis and \code{mqmscan} multiple \QTL\ model mapping.  First
\emph{complete} missing data using the \code{mqmaugment} function (see also
section \ref{augmentation}). After completing the genotypes (without augmentation, because we use \code{minprob=1.0}) information inside the cross object 
(For detailed information on augmentation see section \ref{augmentation}).
...
<<>>=
data(multitrait)
multiaug <- mqmaugment(multitrait,minprob=1.0)
result_multiaug_no_cofactors <- mqmscan(multiaug)
@

After filling in missing genotypes in the dataset 
we scan for \qtl\ using the \mqm\ routines (using default parameters).

We compare these results to those from \code{scanone}, for which
one first calculates conditional QTL genotype probabilities via
\code{calc.genoprob} (also described in section \ref{augmentation}).
...
\mqm\ are similar to \code{scanone}. There are, however, major 
...
\MQM\ introduces pseudo markers at fixed intervals, by default, unlike
\code{scanone}.  A pseudo marker has a name like c7.loc25 - which is the 25
\cM\ pseudo marker on chromosome 7. Note that this reflects the standard naming
used in R/qtl.

The chromosomes are divided into equally spaced sections. Therefore each
chromosome is divided into `fictional markers' spaced equally \code{stepsize}
\cM\ apart. A LOD score for underlying QTLs is calculated at these 
pseudo markers. A small \code{stepsize} allows for smoother profiles compared
with pure marker based mapping approaches. The real markers are listed between
...
<<>>=
just_markers <- mqmextractmarkers(result_multiaug_no_cofactors)
@ 
...
default values. Figure~\ref{AtNoCofs} displayed a large QTL peak on chromosome 4
at 30 \cM. So account for that by setting a cofactor at the marker
nearest to the peak on chromosome 4 and perform a new \code{mqmscan} (see
figures \ref{Cofactor4} and \ref{Cofactor4b}).

\intro{Add marker D4Mit164 (chromosome 4, at 30 \cM) as a cofactor}
...
\intro{Similarly for the multitrait dataset}
<<>>=
...
<<Cofactor4multi>>
@
\caption{\Atintro. A cofactor is added at the top scoring marker of chromosome 5. During the analysis it is kept in the
...
<<Cofactor4bMULTI>>
@
\caption{\Atintro  after introducing a cofactor on chromosome 5 (GH.117C) we see a light increase in LOD scores. 
Compared this figure with Figure~\ref{AtNoCofs}. The black line (MQM) is now above the green dashed line (scanone) \label{Cofactor4b}}
\end{figure}
...
% the second cofactor on chromosome 4. )}
% \end{figure}
...
<<twowaycomparison>>
@
\caption{\Atintro using 1 cofactor (on chromosome 5) versus 2 cofactors (on chromosome 4 and 5)  After introducing a
cofactors at chromosomes 4 there appears to be only a small difference in the likelihood scores for each chromosome. 
The LOD scores do not appear to improve much after adding the second cofactor on chromosome 4. Indicating we have found 
all underlying QTLs for this trait we could add more cofactors (see Figure~\ref{threewaycomparison} However this won't 
improve the current 2 QTL model\label{twowaycomparison}}
\end{figure}
...
@
\caption{\Atintro Comparison of MQM results after QTL mapping using 0, 1 and 2 cofactor(s) at the start of backward elimination \label{threewaycomparison}}
\end{figure}
% ---------------------



When using the functions: \code{mqmsetcofactors} or the automated \code{mqmautocofactor}, the
...
setting cofactors beforehand. When the sample size allows, it is normal procedure to set 
a cofactor at every marker. However current day experiments handle less individuals than genetic locations (markers),therefor only a subset of 
locations can be examined, ( because of lack of sample size (we can set as many cofactors as we have individuals-12)).
The functions: "\code{mqmsetcofactor}" and "\code{mqmautocofactors}" both
create lists consisting of multiple cofactors that can be used for backward elimination. 
\code{mqmautocofactor} is prefered when dealing with low samplesizes, because it takes the underlying marker 
density into account. See Figure~\ref{ManualAuto} for a comparison on the multitrait dataset, using the 
\code{mqmsetcofactors} function to set cofactors every 5th marker and \code{mqmautocofactor} to set 50 cofactors.
After cofactor selection iteratively the MQM algorithm analyses all the markers and drops the least 
informative cofactor from the model. This step is repeated until a limited number of significant cofactors remains.
From Figure~\ref{ManualAuto} we see that when taking marker density into account we get an extra marker that seems 
significant when used as cofactor, compared to the naive approach where we miss this marker because we set cofactors every 5th marker.

After backward elimination \mqm\ scans each chromosome using the model that
includes the retained cofactors. Here we set 50 cofactors using the automatic cofactor routine, 
and we use the naive implementation of setcofactors to set a cofactor at every fifth
marker using \code{mqmsetcofactors}, and assess which chromosomes may be implicated in
the various traits in our multtrait metabolomics dataset (results in Figures~\ref{Backward1} and \ref{Backward2}):
...
<<echo=FALSE>>=
multicofactorlist <- mqmautocofactors(multiaug,50)
autoresultmulti <- mqmscan(multiaug, multicofactorlist)
...
plot(mqmgetmodel(autoresultmulti))
@ 
...
Automatic marker selection (bottom). We see that using \code{mqmautocofactor} does 
place a cofactor at chromosome 1, while this one is missed by mqmsetcofactors
(AFTER backward elimination this extra marker remains significant) \label{ManualAuto}}
\end{figure}
...
<<Backward1multi>>
@
\caption{backward on \Atintro after introducing cofactors at every fifth marker and backward elimination\label{Backward1}}
\end{figure}
...
<<Backward2multi>>
@
\caption{\Atintro Backward elimination using multiple cofactors compared to scanone \label{Backward2}}
\end{figure}
...
\code{cofactor.significance=0.02} chromosomes 4 and 5 are involved. 
...
in the model (the QTLs are still significant). However based on the 
trait under examination and our samplesize, increasing cofactor.significance 
could reduce the number of significant QTLs in our model  \label{FigLowAlpha}}
\end{figure}

%-----------------------

When comparing the \mqm\ scan in Figure~\ref{FigLowAlpha} with the original
\code{scanone} result in Figure~\ref{AtNoCofs} there are some notable differences.
...
\code{mqmplot.singletrait} function (see, for example, Figure~\ref{AutoCofactor}). Here the
...
<<AutoCofactor>>
@
\caption{\Atintro showing the results of trait 1 when using with multiple cofactors, visualized using mqmplot.singletrait \label{AutoCofactor}}
\end{figure}

The second column \code{info} in the result
is calculated from the deviation of the \emph{ideal} marker
distribution. For example, with a dataset of 100 \At, when comparing two
distinct phenotypes at a marker location, we have most power when both groups
are equally divided into 50 \At, and virtually no power when we have one
mouse versus a group of 99 \At. We can multiply the estimated \qtl\ effect by
...
The function \code{mqmplot.directedqtl} may be used to plot LOD
curves with an indication of the sign of the estimated QTL effects.
This requires that \code{mqmscan} was run with
\code{outputmarkers=TRUE}, but that is the default.  For an example on creating directed QTL plots see Figure~\ref{QTLeffects}:
...
<<QTLeffects,eval=FALSE>>=
dirresults <- mqmplot.directedqtl(multiaug,multiresult)
@ 
...
<<MainEffectsD1>>
@
\caption{multitrait, Show effect of marker GH.117C on trait X3.Hydroxypropyl using \code{plot.pgx}\label{MainEffectsD1}}
\end{figure}

%----------------------------------------------

The initial scans for X3.Hydroxypropyl in Figure~\ref{Backward1}
possibly show two main efect \qtl\ on chromosome 4 and 5.  We 
can investigate possible interactions between these main effect QTLs
using the \code{effectplot} function. To investigate the possible 
interaction we select markers GA1 (significant in Figure~\ref{Backward1} and Figure~\ref{Backward2}) 
and GH.117C (significant in Figure~\ref{Backward2} and \ref{AutoCofactor}). See Figure~\ref{epistatic1}.
...
<<epistatic1>>
@
\caption{multitrait, \code{effectplot} shows possible epistatic effects between
markers GH.117C and GA1 \label{epistatic1}}
\end{figure}

%------------------------------------------------


Likewise, in case we are interested in the interactions between the first small hump on
chromosomes 1 (marker: PVV4 not significant) and the main efect on 5 (GH.117C), we could 
make interaction plots between these two
...
<<epistatic2>>
@
\caption{multitrait, \code{effectplot} shows no epistatic effects between
...
<<>>=
require(snow)
results <- mqmpermutation(multiaug,scanfunction=mqmscan,cofactors=multicofactorlist,n.cluster=2,n.perm=25,batchsize=25)
@
\begin{figure}
<<fig=TRUE>>=
mqmplot.permutations(results)
@
\caption{ Multitrait. Calculate significance permuting 25 times over \qtl\ with
...
mqmscanfdr(multifilled,mqmscanall,cofactors=multicofactorlist,n.cluster=2)
@
...
way it is possible to see if significant QTLs for this trait are also colocating with other traits
...
to the centre). Here a 'hotspot' is visible on chromosome 5}
\end{figure}
...
\begin{table}[ht]
  ...
  mqmplot.directedqtl:& Plotting of signle trait with added qtl effect\\
	...
	mqmplot.circle:& Genome plot of QTLs in a circle (optional: Use of location information)\\  
	mqmplot.cistrans:& Genomewide plot of cis- and transQTLs above a threshold \\
