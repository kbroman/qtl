...
These are the typical steps in an \mqm\ \qtl\ analysis using the statistical R language:
...
The downside of augmentation is that many adding possible genotypes can
mushroom the solution space out of available computer memory. Also a large
number of potential genotypes may distort analysis. At this point augmentation
...
are dropped, because of missing data. A future version of \code{mqmaugment} may
take an automatic approach, using imputation with those individuals that can
not be reasonably augmented.

The function \code{mqmaugment} is specific to \mqm\ and the recommended
procedure\footnote{Note that after augmentation the resulting use imputation for dropped individuals object is
...
Now we use a the a real dataset
(multitrait), which contains 24 metabolite traits from a RIL population of \At. 

\intro{Simulate missing data by removing some genotypedata (5\%, 10\% and 80\%) from the crossobject}
...
increase the minprob parameter.  When the \code{minprob} parameter is set too
