...
<<Cofactor4bMULTI>>
@
\caption{\Atinto after introducing a cofactor on chromosome 5 (GH.117C) we see a light increase in LOD scores. \label{Cofactor4b}}
\end{figure}
...
<<AutoCofactor>>
@
\caption{\Atinto showing the results of trait 1 when using with multiple cofactors, visualized using mqmplot.singletrait \label{AutoCofactor}}
\end{figure}
