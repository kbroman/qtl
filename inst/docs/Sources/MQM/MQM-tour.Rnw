...
<<>>=
just_markers <- mqmextractmarkers(mqm_co0)
@ 
...
mqm_co1 <- mqmscan(maug, multicofactorlist)
@
...
multicofactorlist <- mqmsetcofactors(maug,cofactors=multitoset)
mqm_co2 <- mqmscan(maug, multicofactorlist)
@
% -----------
...
% plot(result_2, mqm_co1, col=c("black","green"), lty=1:2)
% @ 
...
\intro{We may plot the results with 0, 1 and 2 cofactors as follows}
...
mqm_auto <- mqmscan(maug, multicofactorlist)
multicofactorlist <- mqmsetcofactors(maug,5)
mqm_backw <- mqmscan(maug, multicofactorlist)
@
...
\intro{ot the results as follows}
