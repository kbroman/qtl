...
<<twowaycomparison>>
@
\caption{\Atintro\ using one cofactor on chromosome 5 (blue), versus two
cofactors on chromosome 4 and 5 (green).  
...
plot(mqm_back,mqm_backw_low, col=c("blue","green"), lty=1:2)
...
<<AutoCofactor>>
@
\caption{\Atintro. Plot using \code{mqmplot.singletrait} of the first trait. The information
content per maker (red), the mapped \qtl\ (black) and the \qtl\ information scores (blue) which is calculated by multiplying marker information content with \qtl\ LOD score.
