...
\citep{Jansen:1994b} } is a valuable addition to the \qtl\ mapper's toolbox. It
...
multitoset <- which.marker(multiaug,"GH.117C")
...
multitoset <- c(multitoset,which.marker(multiaug,find.marker(multiaug,4,10)))
multicofactorlist <- mqmcofactors(multiaug,multitoset)
