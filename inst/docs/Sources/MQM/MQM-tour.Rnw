...
datasets (like the Mouse hyper dataset that comes with R/qtl), these will always be handled with minprob=1.0 (a warning will be issued). A value of \code{minprob=1.0} 
...
figures \ref{AtCofactor4} and \ref{AtCofactor4b}).
...
\intro{Add marker GH.117C as a cofactor (chromosome 5 at 35 \cM)}
...
<<Cofactor4multi>>
@
\caption{\Atintro. A cofactor marker (GH.117C) is added and kept in the
model
\label{AtCofactor4}} 
\end{figure}
...
Plot them together with the \code{scanone} results, as follows:
...
<<Cofactor4bMULTI>>
@
\caption{MULTI4B\label{AtCofactor4b}}
\end{figure}
