...
\title { Tutorial - Multiple-QTL Mapping (MQM) Analysis }
...
\begin{itemize}
\item Optional Calculation of relative marker positions and detection of linkage groups
...
\item If there are no \emph{cofactors\/} defined, The backward elimination of cofactors 
      is not skipped. Perform a genome scan for \qtl testing each genetic (interval) 
...
In an ideal world all datasets would be complete every marker for every 
individual can be determined, however in the real world datasets are often incomplete. That is, genotype
...
routine \code{fill.geno}. As described above, augmentation tries to analyse
...
individual to a second augmentation round when its has too many possible 
genotypes (above \code{maxaugind}). In this second augmentation round the
...
genotypes (upto \code{maxaugind}) or (3) remove the original genotype/individual 
from the analysis.  Note that you can opt to use \code{fill.geno}'s imputation method, 
...
no longer suitable for the use with other R/qtl mapping functions, like \code{scanone} and \cim, because they can not account for
duplicated or dropped of individuals.}.  
...
traits from a RIL population of \At is distributed with
...
(green=5\%, blue=10\%, red=80\%) and \code{scanone}, after \code{fill.geno} 
imputation. The QTL from the \code{scanone} function show a higher LOD score, this is because 
\code{scanone} pretends that the 'filled' genotypes represent the true genotype (which 
is not the case only the most likely genotype is represented).
\label{augment6}}
\end{figure}
\clearpage

\section{Multiple-QTL Mapping (MQM) mapping}
...
\mqm\ are similar to \code{scanone}. However, with augmentation the picture 
changes (see Figure~\ref{AtNoCofs}).
...
the number of individuals-12 \cite{jansen07}.
...
par(mfrow = c(2,1))
mqmplot.cofactors(maug,autocofactors)
mqmplot.cofactors(maug,setcofactors)
@ 
...
\code{mqmpermutation} is nice.  However, for large genome wide association
studies (GWAS), use \code{mqmscanfdr} instead, which estimates false discovery
...
that a confidence interval is provided for each significant \qtl.  The \mqm\
...
genome they were measured (here mQTL for \At), cutoff is at \lod=5}
\end{figure}
...
\begin{table}[ht]
	...
	mqmplot.nice:& plot multiple traits (MQMmulti object) \\
...
Development of an AFLP based linkage map of Ler, Col and Cvi
  ...
  \bibitem{breitling08}
Breitling, R. and Li, Y. and Tesson, B. M. and Fu, J. and Wu, C. and Wiltshire, T. and Gerrits, A. and Bystrykh, L. V. and de Haan, G. and Su, A. I. and Jansen, R. C.; 2008 
