...
% COMMENT: YES !!! vandaar dat we ook mqmautocofactors hebben
...
locations is used, becuse of lack of sample size (we can set as many cofactors as we have individuals-12).  
The functions: "\code{mqmsetcofactor}" and "\code{mqmautocofactors}" both
creates a list consisting of multiple cofactors that can be used for backward elimination. 
