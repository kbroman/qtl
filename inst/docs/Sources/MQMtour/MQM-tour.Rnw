...
individual. Simulate an F$_2$ dataset with 2\% missing markers as
follows:
...
geno.image(mycross)
@
\caption{Simulate an F$_2$ cross with 2\% missing data starting from the
\code{map10} F$_2$ intercross. The figure displays genotypes of 100 individuals 
with around 2\% missing data (white) for the \code{map10} F$_2$ intercross}
\label{missing data}
\end{figure}
...
plot(result_2, result_1, col=c("black","green"), lty=lines)
@
\caption{\hyperintrocolors, after introducing a
