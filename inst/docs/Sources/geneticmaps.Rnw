...
when there are lots of markers.  The function \code{summary.map()}
...
\Sexpr{round(summary.map(mapthis)[1,"max.spacing"],1)}~cM gap; the
...
the function \code{plot.geno()}.  The argument \code{cutoff} indicates
...
plot.geno & Plot genotypes on a particular chromosomes for selected
individuals \\ \hline
%
