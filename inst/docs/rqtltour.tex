...
& \multicolumn{2}{l}{nchr, nind, nmar, nphe, totmar, nmissing} \\ 
...
\item Now let us turn to the fit of multiple-QTL models.  The function
  ...
  Also note that \verb-fitqtl- takes a vector of phenotypes as input,
  and so we pull that from the \verb-hyper- data to make things simpler.

\usercolor \verb|qc <- c(1, 1, 4, 6, 15)| \\
\verb|qp <- c(43.3, 78.3, 30.0, 62.5, 18.0)| \\
\verb|qtl <- makeqtl(hyper, chr=qc, pos=qp)| \\
\verb|phe <- pull.pheno(hyper, 1)| \normalcolor %$
...
\usercolor
\verb|out.fq <- fitqtl(phe, qtl, formula = myformula)| \\
\verb|summary(out.fq)|
\normalcolor
...
\usercolor
\verb|out.fq <- fitqtl(phe, qtl, formula = myformula, drop=FALSE, get.ests=TRUE)| \\
\verb|summary(out.fq)|
\normalcolor
