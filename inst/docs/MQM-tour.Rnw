...
Despite these limitations, \mqm\footnote{Note that \mqm\ is \underline{not the
same} as composite interval mapping (CIM)\cite{jansen94},\cite{CIM}, though
...
package\cite{rqtlbook}\cite{broman09}\cite{broman03} for the R statistical
language\footnote{We assume the reader knows how to load his data into R using
the R/qtl $read.cross$ function, see also the R/qtl tutorials\cite{broman09}
...
\item Fill in missing data, using either $fill.geno$ or $mqmaugmentdata$
\item Unsupervised backward elimination to analyse $cofactors$, using $mqmscan$
\item Optionally select $cofactors$ at markers that are thought to influence \qtl\ at, or near, the location
\item Permutation or simulation analysis to get estimates of significance, using $mqmpermute$
\end{itemize}
...
\item Backward elimination of cofactors, followed by a
      genome scan for \qtl
\item If there are no $cofactors$ defined, perform a
...
the imputation function $fill.geno$ or use the \mqm\ data augmentation routine
$mqmaugment$.  With $fill.geno$ a single 'most likely' genotype is filled in using
...
$mqmaugment$ is specific to \mqm\ and the recommended procedure\footnote{Note
that after augmentation the resulting cross object is no longer suitable for
the use with $scan.one$ or \cim, because of the additional information stored}.

In this tutorial we use \mqm's augmentation. The function $mqmaugment$ fills in
...
$maxaugind$ parameter when there are more missing markers. 

The $minprob$ parameter sets the minimum probability of a genotype for
...
low may result in dropping individuals entirely as $maxaugind$ is quickly
reached footnote{The current version drops individuals that go beyond
$maxaugind$.  This is undesirable behaviour and will be fixed in an upcoming
release. A value $minprob=1.0$ may prevent the dropping of individuals, making
augmentation behave similar to $fill.geno$'s imputation method}.  Use the verbose
option to get more feedback on the augmentation routine and check how many
individuals are dropped. So first we try augmentation with $minprob=1.0$ (figure \ref{augment1}):

\intro{Plot augmented data using $plot.geno$ with $minprob=1.0$}
<<>>=
...
With a lower $minprob$, more \emph{likely} genotypes considered, and the
...
Try augmentation with $minprob=0.1$ (figure \ref{augment2}):
...
The famous mouse hyper tension \emph{hyper} dataset, that comes standard with
R/qtl, is a backcrossed (BC) offspring of mice with two traits in the dataset:
bloodpressure (bp) and sex. Here we show an example of running both $scanone$
single marker \qtl\ analysis and $mqmscan$ multiple \QTL\ model mapping, to map
\qtl\ for blood pressure. First, \emph{complete} missing data:

% \texttt{qtl.mqm} help page into a \LaTeX{} document

\intro{Use $scanone$ and $mqmscan$ after filling missing data with $mqmaugment$}
<<>>=
data(hyper)
...
the $mqmaugment$ routine discussed in section \ref{augmentation}.  Next we scan
for \qtl\ using both $scanone$ and $mqm$ routines (using default parameters).
Figure \ref{NoCofs} shows that with $minprob=1.0$ there is virtually no
...
individuals inside the cross object. If there is any danger of setting too 
many cofactors the two $cofactor$ functions will warn the user by
displaying an error about the user trying to set more cofactors then samplesize 
will permit, and not return a cofactorlist.
...
cofactors until the last added cofactor gets dropped from the model.

Manually determining which marker to set a cofactor can be very time consuming 
in the case of many QTLs underlying a trait. It also can be prone to overfitting 
(because the user looks at the data to decide where to place cofactors). And is 
generally not feasable for a large number of traits. Fortunately to solve 
these issues \mqm\ provides unsupervised backward elimination.

\clearpage
\section{Unsupervised backward elimination}
MQM also provides unsupervised backward elimination on a large number of markers by 
setting cofactors a lot of markers beforehand. When samplesize allows us we could 
place a cofactor at every marker. But in practice ussually only a subset of 
locations is interesting beforehand. Using the $cofactor$ functions user can create
...
marker using $mqmsetcofactors$, and assess which chromosomes may be implicated in
...
Figures can be reconstructed from the result ($scanone$ object) using the
...
\mqm\ scans, as they have an additinal column $info$ which contains the QTL
model. Of interest is that this column also has the \emph{estimated}
information content per marker. $info$ is calculated from the deviation of the
\emph{ideal} marker distribution. 

For example, with a dataset of 100 mice, when comparing two distinct phenotypes
at a marker location, we have most power when both groups are equally divided
into 50 mice, and, virtually, no power when we have a group with 1 mouse versus
a group of 49 mice. We can multiple the estimated \qtl\ effect by this
information content to 'clean' the QTL profile by giving less weight to less
informative markers. Please note that in the sample size already plays a role in 
calculating \qtl. However, despite a lack of formal proof, it can be a way of 
further weighting/exploring \emph{information} content.

\begin{figure}[ht]
<<fig=TRUE>>=
# plot retained info in result with information indicator
mqmplot_one(result, extended=TRUE)
@
\caption{\hyperintro, initially with cofactors at every fifth marker; after backward elimination ($cofactor.significance = 0.002$). $mqmplot\_one$ function
shows retained information in $result$ with added weighting of information at
marker positions
}
\label{AutoCofactor}
\end{figure}
...
from our model ) we can use permutation testing provided by the function $mqmpermute$. 
This step is computationally expensive - repeating the same test thousands of 
times on shuffled data to see what LOD scores come up in non associated data. 
We can use two 'favors' to estimate QTL significance when dealing with a small 
amounts of traits. We can use the $bootmethod$ parameter of the $mqmpermute$ 
function to choose the 'flavor' of permutation $0 = non-parametric$ permutation or 
$1 = parametric bootstrap$ a simulation of the data. If you have access to multiple 
CPUs on your computer you can install the SNOW package \cite{tierney03}\cite{tierney04}, 
which allows parallel computations on multiple CPU (cores). For small sets, 
with a limited amount of classical traits, the $mqmpermute$ works well. For large 
genome wide association studies (GWAS) use $mqmscanfdr$ to estimate the FDR across the entire
dataset at \lod\ cutoffs levels. Both routines share some parameters: like $b.size$ the batchsize per core. 
...
\end{figure}
The next plot is the $mqmplot\_circle$ function. It shows a circular representation 
of the genome. After using automatic backward selection we can also highlight markers
found to be significant using MQM. When highlighting the size of the markers is scaled
based on the LODscore of the marker for the highlighted trait. When not highlighting 
markers are not scaled. When plotting multiple traits we can see all QTLs of all the traits
outside the chromosome (solid colored squares, or when highlighting in gray)
\begin{figure}[ht]
<<fig=TRUE>>=
...
\end{figure}
When having locations we can also use the $mqmplot_circle$ function.
\begin{figure}[ht]
<<fig=TRUE>>=
mqmplot_circle(multiloc,resall, highlight=2)
@
\caption{Circleplot 3 - Multiple traits with locations added, here we highlight trait number 2}
\end{figure}
...
\begin{thebibliography}{9}
  ...
  \bibitem{jansen94b}
	  Jansen R.C.
		\emps{Controlling the Type I and Type II Errors in Mapping Quantitative Trait Loci}.
  ...
  \bibitem{CIM}
	  Composite Interval Mapping reference \FIXME
\end{thebibliography}
\end{document}
