...
setting cofactors beforehand. When $samplesize$ allows it a 
a cofactor is set at every marker. However, normally only a subset of 
locations is of interest. The $cofactor$ function creates
...
The $mqmgetmodel$ function returns the final model from the resulting $scanone$ type
...
\clearpage
\intro{Plot result of MQM backward elimination against that of $scanone$}
\begin{figure}[ht]
<<fig=TRUE>>=
...
\intro{Plot with lowered $cofactor.significance$}
\begin{figure}[ht]
<<fig=TRUE>>=
...
$scanone$ result in figure \ref{NoCofs} there are some notable differences.
...
$mqmcan$, as they have an additional column $mqm$ which contains the \qtl\
model.  This extra column/list also contains the \emph{estimated} information
content per marker.  

The column $mqm$ is calculated from the deviation of the \emph{ideal} marker
...
Effects can be add to plots using $mqmplot\_directedqtl$, which requires
setting $outputmarkers=TRUE$ with $mqmscan$ (default).  To remove plotting the
virtual markers introduced by \mqm\ use $mqmextractmarkers(result)$. The
...
can use \rqtl's standard plotting tools to visualize $main$ and/or $epistatic$
...
@
\caption{\hyperintro. Show effect of marker D1Mit102 using $plot.pgx$}
\label{MainEffectsD1}
\end{figure}
...
@
\caption{\hyperintro. $effectplot$ shows possible small epistatic effects between
...
@
\caption{\hyperintro. $effectplot$ shows an epistatic effect between markers D1Mit102 and D5Mit213}
\label{epistatic2}
\end{figure}
