...
\verbatim{http://cran.r-project.org/web/packages/snow/index.html}. Once the package 
...
calls to $mqmscan$. In addition special plots are presented for xQTL studies.

As an example, the mQTL dataset $multitrait$,
...
The next plot is the $mqmplot_circle$. It shows a circular
...
The next plot is $mqmplot\_cistrans$. This plot is only available
