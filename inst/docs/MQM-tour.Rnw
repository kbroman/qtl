...
model) permutation testing is provided by the function $mqmpermute$. This step
...
significance. The $bootmethod$ parameter of the $mqmpermute$ function selects
...
computations on multiple CPU/cores. 
...
$cofactor.significance=0.05$ (green) or $cofactor.significance=0.10$ (blue)).
...
For small datasets, with a limited amount of classical traits, $mqmpermute$
works fine. For large genome wide association studies (GWAS) use $mqmscanfdr$
...
To estimate FDR, whole genome information is permuted with $mqmscanfdr$. This
...
the traits and counts observed \qtl\ (markers with a LOD above $x$) when
...
The function $mqmpermutate$ does single trait permutations, and does not take
...
converted to the standard R/qtl format using the $mqmprocesspermutation$
function. The resulting object is of class $scanoneperm$ and can be used by the
...
\section{Parallelized MQM multiple trait analysis}

MQM can analyse traits simultaneously using multiple CPU/cores.

For example, the mQTL dataset $multitrait$,
...
the $pheno.col$ parameter. To map back the 
...
Use $mqmplot\_nice$ for more graphical output. (Unfortunately this does not
...
The following plot is the $mqmplot\_circle$ function. It shows a circular
representation of the genome. After using automatic backward selection
highlighted markers are found to be significant using \mqm. When highlighting the
size of the markers is scaled based on the LOD score of the marker for the
highlighted trait. When not highlighting markers are not scaled. When plotting
multiple traits we can see all QTLs of all the traits outside the chromosome
(solid colored squares, or when highlighting in gray)
...
When having locations we can, again, use the $mqmplot_circle$ function, now with 
