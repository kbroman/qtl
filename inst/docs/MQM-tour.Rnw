...
Multiple QTL model (\mqm) mapping uses an automated model selection method for
mapping \qtl, as described by R.C. Jansen\cite{jansen94}\footnote{Note that
\underline{\mqm\ is not the same the same as composite interval mapping
(CIM)}\cite{jansen94},\cite{CIM}, though both methods were introduced around
the same time}.
...
Despite these limitations, \mqm\ is a valuable addition to the \qtl\ mapper's
toolbox. It is able to deal with interference of \qtl, handles missing data and
allows more precise detection than other methods. This tutorial will show how
to use \mqm\ for \qtl\ mapping. Also R/qtl's \mqm\ is faster than other
implementations and scales on multi-CPU systems and computer clusters.
...
\item Select $cofactors$ at markers that are suspected to have a \qtl\ at or near their location
\item Use $mqmscan$ with unsupervised backward elimination to analyse the $cofactors$
\item Then use $mqmpermute$ with a permutation or simulation analysis to get estimates of significance
\end{itemize}

Using (restricted) maximum likelihood  the algorithm employs a backward
elimination strategy to identify \qtl\ underlying the trait. The algorithm 
...
default of 60 is usually too little enough (3 to 4 missing markers per individual).
This was set low to prevent too much memory use because space is allocated prior 
to augmentation. As a result the user has to increase the $maxaugind$ parameter when 
there are more missing markers. Each individual is augmented with the most likely genotype
, then other genotypes that are still more likely then $minprob$ are also included.
in other words, only include augmented genotypes of individuals that are
above a probability of occurring (and do not include the less likely cases). Note that
